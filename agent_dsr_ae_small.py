...
def PrintStat(elapsed_time, step, step_num, train_scores):
    ...
    remain_s = int(
        (steps_remain - remain_h * steps_per_h - remain_m * steps_per_m)
        / steps_per_s
    )
    ...
    print(
        "{}% | Steps: {}/{}, {:.2f}M step/h, {:02}:{:02}:{:02}/{:02}:{:02}:{:02}".format(
            100.0 * step / step_num,
            step,
            step_num,
            steps_per_h / 1e6,
    ...
    print(
        "Episodes: {} Rewards: mean: {:.2f}, std: {:.2f}, min: {:.2f}, max: {:.2f}".format(
...
class Model(object):
    def __init__(self, session, actions_count, scope):
        ...
        self.s_ = tf.placeholder(
            shape=[None] + list(resolution), dtype=tf.float32
        )
        ...
        conv2 = tf.contrib.layers.conv2d(
            conv1, num_outputs=16, kernel_size=[3, 3], stride=[2, 2]
        )  # , padding='VALID')
        conv2_flat = tf.contrib.layers.flatten(conv2)
        self.fc1 = tf.contrib.layers.fully_connected(
            conv2_flat, num_outputs=num_layers
        )
        ...
        self.reconstruct = tf.contrib.layers.conv2d_transpose(
            deconv1,
            num_outputs=3,
            kernel_size=[3, 3],
            stride=[2, 2],
        ...
        with tf.variable_scope("regression") as s:
            self.r = tf.contrib.layers.fully_connected(
                self.fc1, num_outputs=1, activation_fn=None, scope=s
            )
            # self.q = tf.contrib.layers.fully_connected(mu, num_outputs)

        def add_sr_layer():
            x = tf.contrib.layers.fully_connected(
                self.fc1_stopgrad, num_outputs=num_layers
            )
        ...
        self.loss_sr = tf.losses.mean_squared_error(
            self.mu_, tf.gather_nd(self.mu, self.a_)
        )
    ...
    def GetMU(self, state, action):
        ...
        return self.session.run(
            self.getmu, feed_dict={self.s_: state, self.a_: action}
        )
...
class Agent(object):
    ...
    def LearnFromMemory(self, mode):
        ...
        if (
            self.memory.size > 2 * replay_memory_batch_size
            and self.nonzeroMemory.size > 2 * replay_memory_batch_size
        ):
            # print("sampling")
            if random.random() <= 0.3:
                s1, a, s2, isterminal, r = self.nonzeroMemory.Get(
                    replay_memory_batch_size
                )
            else:
                s1, a, s2, isterminal, r = self.memory.Get(
                    replay_memory_batch_size
                )
            ...
            mu2 = self.model.GetMU(
                ...
                np.asarray(
                    zip(
                        np.arange(replay_memory_batch_size),
                        itertools.repeat(self.model.GetAction(s2)),
                    )
                ),
            )
            ...
            done = (1 - isterminal).reshape(
                (replay_memory_batch_size, 1)
            ) + np.zeros((replay_memory_batch_size, num_layers))

            self.model.Learn(
                self.model.GetPhi(s1) + done * discount_factor * mu2,
                s1,
                a,
                r,
                mode,
    ...
    def Step(self, iteration):
        ...
        cv2.imshow(
            "reconstruction",
            self.model.GetReconstruction(np.reshape(s, (1, 40, 40, 3)))[0],
        )
        ...
        for step in xrange(1, step_num + 1):
            ...
            if not env.IsRunning():
                summary = self.session.run(
                    ...
                    feed_dict={self.model.reward_per_episode: self.rewards},
                )
            ...
            if step % save_each == 0:
                model_name_curr = model_name + "_{:04}".format(
                    int(step / save_each)
                )
                print(
                ...
                PrintStat(
                    time.time() - start_time, step, step_num, train_scores
                )
...
def Test(agent):
    if test_write_video:
        ...
        out_video = cv2.VideoWriter(
            path_work_dir + "test.avi", fourcc, fps, size
        )
