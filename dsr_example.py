...
class DSRModel(Model):
    ...
    def create_feature_model(
        ...
        features_phi_state = Dense(512, activation='')(x)
        rewards_state = Dense(self.num_actions, activation='linear')(features_phi_state)

        model = Model(inputs=input_layer, outputs=rewards_state)
        opt_sgd = SGD(learning_rate=self.learning_rate)
        model.compile(optimizer=opt_sgd)

        self.features_phi_state = features_phi_state
